...
test.describe('고정비 관리', () => {
  ...
  test('고정비 수정', async ({ page }) => {
    ...
    // 수정 버튼 클릭
    await page.getByText(/월세/i).locator('..').getByRole('button', { name: /수정/i }).click();
    ...
    // 수정된 내용 확인
    await expect(page.getByText(/600,000원/i)).toBeVisible();
  });
  ...
  test('고정비 삭제', async ({ page }) => {
    ...
    // 삭제 버튼 클릭
    await page.getByText(/통신비/i).locator('..').getByRole('button', { name: /삭제/i }).click();
